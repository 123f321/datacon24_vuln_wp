...
from arbitrary import check_arbitrary
from bypass import check_bypass
from int_overflow import check_int_overflow
from bof import check_bof
from others import check_others,check_output_regex
...
#key_env='6ad9127594fa1951b18de59fa8ecb856'
...
HumanPromptStep1 = PromptTemplate(
    template="""
                ...
                "Dangerous function name list": the function names of the {slice_vul} vulnerability type in the code snippet.
                "Other related function name list": other function names related to the dangerous function in the code snippet.
                ...
                3. If the code snippet provided is not related to the {slice_vul} vulnerability, the output "dangerous function name list" is NULL, and the "other related function name list" is also NULL;
                4. Please analyze from the perspective of the function call chain Analyze the code snippet provided, and save other functions in the same function call chain as the dangerous function in the "other related function name list";
                ...
                8. Please do not output irrelevant function names to the results. You are not completing a binary classification task, but filtering the functions we need;
...
response_schemas_step1 = [
    ResponseSchema(name="vul_functions", description="Dangerous function name list", type="list[str]"),
    ResponseSchema(name="related_functions", description="Other related function name list", type="list[str]")
]
...
def run_step2(slice_vul,file_content):
    json_out_step2=None
...
def step_by_step(type, subdir_path, answer_dir):
    ...
    if file_num>1 or len(file_content)>10000:#split
        functions=split_files(subdir_path,ext)
    ...
    if file_num<=1 and len(file_content)<=14000:
        ...
        run_step2(slice_vul,file_content)
            ...
            for danger in dangerous_funcs:
                if danger in head:
                    if len(file_content2)+len(function)>14000:
                        print("full")
                        run_step2(slice_vul,file_content2)
                        file_content2=function
                    file_content2+=function
                    has_danger=True
            if has_danger:
                dangerous_funcs.remove(danger)
